import type { ProviderConfig, ProviderDefinition, ProviderModel } from '@/server/providers/types'
import { createLogger } from '@/server/logger'

const log = createLogger('provider:fireworks')

interface FireworksModel {
  id: string
  object: string
  owned_by?: string
}

interface FireworksModelsResponse {
  data?: FireworksModel[]
}

function classifyModel(model: FireworksModel): 'llm' | 'embedding' | 'image' | null {
  const id = model.id.toLowerCase()

  // Image generation models
  if (id.includes('flux') || id.includes('stable-diffusion') || id.includes('sdxl') || id.includes('playground-v2')) return 'image'

  // Embedding models
  if (id.includes('embed') || id.includes('bge') || id.includes('nomic') || id.includes('gte-') || id.includes('e5-')) return 'embedding'

  // LLM models
  if (
    id.includes('llama') ||
    id.includes('mistral') ||
    id.includes('mixtral') ||
    id.includes('qwen') ||
    id.includes('deepseek') ||
    id.includes('gemma') ||
    id.includes('yi-') ||
    id.includes('phi-') ||
    id.includes('starcoder') ||
    id.includes('command') ||
    id.includes('firefunction') ||
    id.includes('mythomax')
  ) return 'llm'

  // Fallback: assume LLM
  return 'llm'
}

async function fetchModels(config: ProviderConfig): Promise<FireworksModel[]> {
  const baseUrl = config.baseUrl ?? 'https://api.fireworks.ai/inference/v1'
  const response = await fetch(`${baseUrl}/models`, {
    headers: {
      'Authorization': `Bearer ${config.apiKey}`,
    },
  })

  if (!response.ok) {
    throw new Error(`Fireworks AI API error: ${response.status}`)
  }

  const data = await response.json()
  if (Array.isArray(data)) return data as FireworksModel[]
  return (data as FireworksModelsResponse).data ?? []
}

export const fireworksProvider: ProviderDefinition = {
  type: 'fireworks',
  capabilities: ['llm', 'embedding', 'image'],

  async testConnection(config: ProviderConfig) {
    try {
      const models = await fetchModels(config)
      const valid = models.length > 0
      log.info({ valid, modelCount: models.length }, 'Connection test completed')
      return { valid }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Connection failed'
      log.error({ err: error }, 'Connection test failed')
      return { valid: false, error: message }
    }
  },

  async listModels(config: ProviderConfig) {
    try {
      const apiModels = await fetchModels(config)
      const models = apiModels
        .map((m): ProviderModel | null => {
          const capability = classifyModel(m)
          if (!capability) return null
          return {
            id: m.id,
            name: m.id,
            capability,
          }
        })
        .filter((m): m is ProviderModel => m !== null)
        .sort((a, b) => a.id.localeCompare(b.id))
      log.debug({ count: models.length }, 'Models listed')
      return models
    } catch (error) {
      log.error({ err: error }, 'Failed to list models')
      return []
    }
  },
}
